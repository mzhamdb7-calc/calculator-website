...
/* ===== Age Calculator final single renderer ===== */
(function () {
  'use strict';
  ...
  function parseISO(value, useNowIfToday) {
    ...
    if (useNowIfToday && value === todayISO()) {
      var now = new Date();
  ...
  function breakdown(start, end) {
    ...
    var h = end.getHours() - start.getHours();
  ...
  function nextAgeCountdownData(birth, target) {
    var next = new Date(target.getFullYear(), birth.getMonth(), birth.getDate(), birth.getHours(), birth.getMinutes(), birth.getSeconds(), birth.getMilliseconds());
    if (next <= target) next = new Date(target.getFullYear() + 1, birth.getMonth(), birth.getDate(), birth.getHours(), birth.getMinutes(), birth.getSeconds(), birth.getMilliseconds());
    return {
      age: Math.max(0, next.getFullYear() - birth.getFullYear()),
      seconds: Math.max(0, Math.floor((next.getTime() - target.getTime()) / 1000)),
      days: Math.max(0, Math.ceil((next.getTime() - target.getTime()) / 86400000))
    };
  }
  function countdownToAge(birth, target, age, label) {
    var ageDate = new Date(birth.getFullYear() + age, birth.getMonth(), birth.getDate());
    if (target >= ageDate) return label + ' reached';
    var days = Math.ceil((ageDate.getTime() - target.getTime()) / 86400000);
    return formatLarge(days) + ' days before ' + label;
  }
  ...
  function leapInfo(birth, target) {
    ...
    var count = 0;
    for (var y = birthYear; y <= target.getFullYear(); y += 1) if (isLeapYear(y)) count += 1;
    if (birth.getMonth() === 1 && birth.getDate() === 29) return 'Leap day birthday; leap years passed: ' + count;
    return 'Born in leap year: ' + (isLeapYear(birthYear) ? 'Yes' : 'No');
  }
  function row(label, value) {
    return '<li class="age-single-result-row"><strong>' + esc(label) + ':</strong><span>' + esc(value) + '</span></li>';
  }
  function group(title, rows) {
    return '<section class="age-single-group-box"><h3>' + esc(title) + '</h3><ul class="age-single-result-list">' + rows.join('') + '</ul></section>';
  }
  function ensureOnlyOnePanel() {
    if (!isAgePage()) return null;
    ...
    var allPanels = Array.from(document.querySelectorAll('#ageReportOutput, #ageResult, .age-clean-result, .age-point-output, .age-final-output')).filter(function (el) {
      return el && !el.closest('#calculatorReportPage');
    });
    var panel = document.getElementById('ageReportOutput') || allPanels[0];
    if (!panel) {
  ...
  function syncResultWidth() {
    var main = document.querySelector('main.age-calculator-container');
    var calc = main && main.querySelector(':scope > .calculator');
    var panel = document.getElementById('ageReportOutput');
    if (!main || !calc || !panel) return;
    if (panel.parentElement !== main || panel.previousElementSibling !== calc) {
      calc.insertAdjacentElement('afterend', panel);
    }
    var width = calc.getBoundingClientRect().width;
  ...
  function showPanel(html) {
    ...
    syncResultWidth();
    setTimeout(syncResultWidth, 60);
  }
  function calculateAgeSingle() {
    ...
    if (!birth || !target || birth > target) {
      showPanel('<div class="age-single-result-shell"><div class="age-single-error">Please choose a valid birth date before the calculation date.</div></div>');
      return;
    }
    ...
    var totalMs = target.getTime() - birth.getTime();
    var totalDays = Math.max(0, Math.floor(totalMs / 86400000));
    var totalSeconds = Math.max(0, Math.floor(totalMs / 1000));
    ...
    var nextAge = nextAgeCountdownData(birth, target);
    var daysToNext = nextAge.days;
    var exactText = exact.years + ' years, ' + exact.months + ' months, ' + exact.days + ' days, ' + exact.hours + ' hours, ' + exact.minutes + ' minutes';
    var title = formatLarge(nextAge.seconds) + ' second to ' + nextAge.age + ' years old';

    var html = '<div class="age-single-result-shell">' +
      '<h2 class="age-single-result-title">' + esc(title) + '</h2>' +
      '<div class="age-single-result-grid">' +
        group('Birth & calendar', [
          ...
          row('Born date in Islamic calendar', intlDate(birth, 'en-GB-u-ca-islamic')),
          row('Born date in Chinese calendar', intlDate(birth, 'en-GB-u-ca-chinese'))
        ]) +
        group('Current age', [
          row('Exact age', exactText),
        ...
        group('Birthday & milestones', [
          row('Next birthday countdown', daysToNext + ' day' + (daysToNext === 1 ? '' : 's')),
          row('Legal age', countdownToAge(birth, target, 18, 'legal adult age')),
          row('Retirement', countdownToAge(birth, target, 60, 'retirement')),
          row('Leap year age', leapInfo(birth, target))
        ]) +
        group('Life summary', [
  ...
  function bindAgeSingleRenderer() {
    if (!isAgePage()) return;
    ensureOnlyOnePanel();
    ...
    ['ageName', 'birthdate', 'dateToCalculate'].forEach(function (id) {
      var el = $(id);
      if (!el || el.dataset.ageSingleBound === '1') return;
      el.dataset.ageSingleBound = '1';
      el.addEventListener('input', calculateAgeSingle);
      el.addEventListener('change', calculateAgeSingle);
    });

    window.calculateAge = calculateAgeSingle;
    window.calculateAgeStandalone = calculateAgeSingle;
    calculateAgeSingle();
    syncResultWidth();
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', bindAgeSingleRenderer);
  else bindAgeSingleRenderer();
  window.addEventListener('load', function () { bindAgeSingleRenderer(); syncResultWidth(); });
