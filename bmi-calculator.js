...
/* =========================================================
   Page-local mobile/navigation JS migrated from phone.js.
   Every HTML page now loads its own JS file only.
   ========================================================= */
...
(function () {
  ...
  function placeBmiResultPanel(panel) {
    if (!panel || !isBmiPage()) return;
...
/* ===== BMI DIRECT RESULT VALUES FIX: no placeholder result ===== */
(function () {
  'use strict';

  if (window.__bmiDirectResultValuesFixInstalled) return;
  window.__bmiDirectResultValuesFixInstalled = true;
  ...
  function unit() {
    var btn = byId('unitToggleBtn');
    return String((btn && (btn.dataset.currentUnit || btn.textContent)) || (document.body && document.body.dataset.bmiUnit) || 'si').toLowerCase().includes('us') ? 'us' : 'si';
  }

  function displayWeightFromKg(kg, activeUnit) {
    if (!Number.isFinite(kg)) return 'Not provided';
    if (activeUnit === 'us') return formatNumber(kg / 0.45359237, 1) + ' lb';
    return formatNumber(kg, 1) + ' kg';
  }

  function measurementDisplay(value, activeUnit) {
    if (!Number.isFinite(value) || value <= 0) return 'Not provided';
    return formatNumber(value, 1) + (activeUnit === 'us' ? ' in' : ' cm');
  }
  ...
  function bmiCategory(bmi) {
    if (!Number.isFinite(bmi)) return 'Not provided';
  ...
  function getHealthRisk(category, ratio) {
    var text = 'Average risk — use BMI with waist-to-height ratio for a better view';
    if (category === 'Normal' && (!Number.isFinite(ratio) || ratio < 0.5)) text = 'Lower risk range';
    if (category === 'Underweight') text = 'Possible risks: low energy, nutrient deficiency, weaker immunity, and bone health concerns';
    if (category === 'Overweight') text = 'Possible risks: higher blood pressure, insulin resistance, fatty liver, joint strain, and higher cholesterol';
    if (category === 'Obese') text = 'Possible risks: type 2 diabetes, high blood pressure, heart disease, sleep apnea, fatty liver, and joint problems';
    if (Number.isFinite(ratio) && ratio >= 0.5) text += '; waist-to-height ratio suggests higher central-body-fat risk';
    return text;
  }
  ...
  function getMainAndCalculator() {
    var main = document.querySelector('main.bmi-calculator-container');
    var calc = main && main.querySelector(':scope > .calculator.bmi-calculator-box');
    return { main: main, calc: calc };
  }

  function getResultPanel() {
    var parts = getMainAndCalculator();
    if (!parts.main || !parts.calc) return null;
    var panel = byId('bmiReportOutput') || document.querySelector('.bmi-rebuilt-output, .bmi-clean-result, .bmi-box-output');
    if (!panel) {
      panel = document.createElement('section');
      panel.id = 'bmiReportOutput';
      panel.setAttribute('aria-label', 'BMI result');
    }
    panel.id = 'bmiReportOutput';
    panel.className = 'loan-style-output-panel calculator-clean-result bmi-clean-result bmi-box-output bmi-rebuilt-output bmi-direct-output';
    if (panel.parentElement !== parts.main || panel.previousElementSibling !== parts.calc) {
      parts.calc.insertAdjacentElement('afterend', panel);
    }
    panel.style.setProperty('grid-column', window.innerWidth > 850 ? '2' : '1', 'important');
    panel.style.setProperty('grid-row', window.innerWidth > 850 ? '2' : 'auto', 'important');
    panel.style.setProperty('position', 'static', 'important');
    panel.style.setProperty('display', 'block', 'important');
    panel.style.setProperty('visibility', 'visible', 'important');
    panel.style.setProperty('opacity', '1', 'important');
    panel.style.setProperty('margin', '24px 0 0', 'important');
    panel.style.setProperty('padding', '0', 'important');
    panel.style.setProperty('box-sizing', 'border-box', 'important');
    var width = Math.round(parts.calc.getBoundingClientRect().width);
    if (width > 0 && window.innerWidth > 850) {
  ...
  function makeSummary(row, title) {
    if (!row) return '';
    return '<article class="bmi-rebuilt-summary-item"><span data-bmi-label>' + esc(title || row.label) + '</span><strong data-bmi-value>' + esc(row.value) + '</strong></article>';
  }
  ...
  function makeGroup(title, rows) {
    ...
    return '<section class="bmi-rebuilt-group"><h3>' + esc(title) + '</h3><ul>' + rows.map(makeRow).join('') + '</ul></section>';
  }

  function findRow(rows, label) {
    return rows.find(function (row) { return row.label.toLowerCase() === label.toLowerCase(); });
  }

  function openBmiReport(rows) {
    var reportHtml = '<!doctype html><html><head><meta charset="utf-8"><title>BMI Report</title><meta name="viewport" content="width=device-width, initial-scale=1"><style>' +
      'body{font-family:Arial,sans-serif;margin:0;background:#f6faf8;color:#14251f;padding:28px}main{max-width:900px;margin:auto;background:#fff;border:1px solid #cfe2da;border-radius:20px;padding:28px}h1{margin:0 0 8px;font-size:32px}.date{color:#5f756c;margin:0 0 24px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.row{border:1px solid #dbece5;border-radius:12px;padding:12px}.label{display:block;color:#5a6e66;font-weight:800;font-size:12px;text-transform:uppercase;letter-spacing:.06em}.value{font-size:18px;font-weight:800;margin-top:6px}@media print{body{background:#fff;padding:0}main{border:0;border-radius:0}.actions{display:none}}@media(max-width:700px){.grid{grid-template-columns:1fr}}button{border:0;border-radius:999px;background:#0b8f73;color:#fff;padding:12px 18px;font-weight:800;margin-bottom:18px;cursor:pointer}' +
      '</style></head><body><main><div class="actions"><button onclick="window.print()">Print / Save PDF</button></div><h1>BMI Report</h1><p class="date">Generated ' + esc(new Date().toLocaleString()) + '</p><div class="grid">' +
      rows.map(function (row) { return '<div class="row"><span class="label">' + esc(row.label) + '</span><div class="value">' + esc(row.value) + '</div></div>'; }).join('') +
      '</div></main></body></html>';
  ...
  function calculateRows() {
    var activeUnit = unit();
    var weight = numberOf('weight');
    var height = numberOf('height');
    if (!Number.isFinite(weight) || !Number.isFinite(height) || weight <= 0 || height <= 0) return null;

    var waist = numberOf('waist');
    var neck = numberOf('bmiNeck');
    var wrist = numberOf('bmiWrist');
    var shoulder = numberOf('bmiShoulder');
    var hip = numberOf('bmiHip');
    var age = numberOf('bmiAge');
    var gender = valueOf('bmiGender') || 'male';
    var activity = valueOf('bmiActivityLevel') || 'moderate';
    var name = valueOf('bmiName');
    var targetWeight = numberOf('bmiTargetWeight');
    var timeGoalAmount = numberOf('bmiTimeGoalAmount');
    var timeGoal = valueOf('bmiTimeGoal') || 'weekly';

    var weightKg = activeUnit === 'us' ? weight * 0.45359237 : weight;
    var heightCm = activeUnit === 'us' ? height * 2.54 : height;
    var heightM = heightCm / 100;
    var bmi = weightKg / (heightM * heightM);
    var category = bmiCategory(bmi);
    var healthyMinKg = 18.5 * heightM * heightM;
    var healthyMaxKg = 24.9 * heightM * heightM;
    var healthyRange = displayWeightFromKg(healthyMinKg, activeUnit) + ' – ' + displayWeightFromKg(healthyMaxKg, activeUnit);
    var difference = 'Inside healthy range';
    if (weightKg < healthyMinKg) difference = 'Gain about ' + displayWeightFromKg(healthyMinKg - weightKg, activeUnit) + ' to enter healthy range';
    if (weightKg > healthyMaxKg) difference = 'Lose about ' + displayWeightFromKg(weightKg - healthyMaxKg, activeUnit) + ' to enter healthy range';

    var ratio = Number.isFinite(waist) && waist > 0 ? waist / height : NaN;
    var waistStatus = Number.isFinite(ratio) ? (ratio < 0.5 ? 'Healthy' : 'Higher risk') : 'Enter waist to check';
    var healthRisk = getHealthRisk(category, ratio);

    var activityFactors = { sedentary: 1.2, light: 1.375, moderate: 1.55, active: 1.725, veryActive: 1.9 };
    var caloriesMaintain = 'Enter age to estimate calories/day';
    var caloriesGain = 'Enter age to estimate calories/day';
    var caloriesLoss = 'Enter age to estimate calories/day';
    var bodyFatText = 'Enter age and gender to estimate body fat';
    if (Number.isFinite(age) && age > 0) {
      var maleBmr = 10 * weightKg + 6.25 * heightCm - 5 * age + 5;
      var femaleBmr = 10 * weightKg + 6.25 * heightCm - 5 * age - 161;
      var bmr = gender === 'male' ? maleBmr : gender === 'female' ? femaleBmr : (maleBmr + femaleBmr) / 2;
      var maintain = Math.round(bmr * (activityFactors[activity] || activityFactors.moderate));
      caloriesMaintain = maintain.toLocaleString('en-US') + ' calories/day to maintain weight';
      caloriesGain = (maintain + 500).toLocaleString('en-US') + ' calories/day to add weight';
      caloriesLoss = Math.max(1200, maintain - 500).toLocaleString('en-US') + ' calories/day to lose weight';
      var bodyFatNumber = 1.2 * bmi + 0.23 * age - 10.8 * (gender === 'male' ? 1 : gender === 'female' ? 0 : 0.5) - 5.4;
      bodyFatText = Math.max(0, bodyFatNumber).toFixed(1) + '% estimated body fat';
    }

    var wristCm = activeUnit === 'us' && Number.isFinite(wrist) ? wrist * 2.54 : wrist;
    var shoulderCm = activeUnit === 'us' && Number.isFinite(shoulder) ? shoulder * 2.54 : shoulder;
    var hipCm = activeUnit === 'us' && Number.isFinite(hip) ? hip * 2.54 : hip;
    var waistCm = activeUnit === 'us' && Number.isFinite(waist) ? waist * 2.54 : waist;

    var frameSize = 'Not provided';
    if (Number.isFinite(wristCm) && wristCm > 0) {
      var wristHeightRatio = wristCm / heightCm;
      frameSize = gender === 'female'
        ? (wristHeightRatio < 0.086 ? 'Small frame' : wristHeightRatio <= 0.094 ? 'Medium frame' : 'Large frame')
        : (wristHeightRatio < 0.095 ? 'Small frame' : wristHeightRatio <= 0.104 ? 'Medium frame' : 'Large frame');
    }

    var bodyShape = 'Add shoulder, waist, and hip to estimate body shape';
    if (Number.isFinite(shoulderCm) && Number.isFinite(waistCm) && Number.isFinite(hipCm) && waistCm > 0 && hipCm > 0) {
      bodyShape = shoulderCm >= 1.08 * hipCm && waistCm <= 0.78 * shoulderCm ? 'Inverted triangle / V-shape' :
        hipCm >= 1.08 * shoulderCm && waistCm <= 0.78 * hipCm ? 'Pear / lower-body dominant shape' :
        Math.abs(shoulderCm - hipCm) <= Math.max(4, 0.06 * hipCm) && waistCm <= 0.78 * Math.min(shoulderCm, hipCm) ? 'Hourglass / balanced shape' :
        waistCm >= 0.9 * Math.min(shoulderCm, hipCm) ? 'Apple / midsection-dominant shape' : 'Rectangle / straight balanced shape';
    }

    var fatDistribution = 'Add waist and hip to estimate fat distribution';
    if (Number.isFinite(waistCm) && Number.isFinite(hipCm) && hipCm > 0) {
      var whr = waistCm / hipCm;
      fatDistribution = ((gender === 'male' ? whr >= 0.9 : whr >= 0.85) ? 'Central / abdominal fat pattern' : 'Lower central-fat pattern') + ' (waist-to-hip ratio ' + whr.toFixed(2) + ')';
    } else if (Number.isFinite(ratio)) {
      fatDistribution = ratio >= 0.5 ? 'Central-body-fat risk pattern from waist-to-height ratio' : 'Lower central-fat pattern from waist-to-height ratio';
    }

    var somatotype = bmi < 18.5 && frameSize !== 'Large frame' ? 'Ectomorph tendency' : bmi >= 25 ? 'Endomorph tendency' : frameSize === 'Large frame' ? 'Mesomorph tendency' : 'Balanced mixed tendency';
    var bodyType = /ectomorph/i.test(somatotype) ? 'Lean frame' : /mesomorph/i.test(somatotype) ? 'Athletic / solid frame' : /endomorph/i.test(somatotype) ? 'Softer / higher-storage frame' : 'Balanced frame';

    var suggestedExercise = /overweight|obese|endomorph/i.test(category + ' ' + somatotype)
      ? 'Low-impact cardio 3–5 days/week, full-body resistance training 2–3 days/week, daily walking, and core stability work.'
      : /underweight|ectomorph/i.test(category + ' ' + somatotype)
        ? 'Strength training 3–4 days/week, progressive overload, compound lifts, light cardio, and enough rest for muscle gain.'
        : 'Balanced plan: strength training 3 days/week, cardio 2 days/week, mobility work, and gradual weekly progression.';
    var suggestedFoods = /overweight|obese|endomorph/i.test(category + ' ' + somatotype)
      ? 'Prioritize high-protein and high-fiber meals, vegetables, fruit, water, and reduce fried food, sweets, and sweet drinks.'
      : /underweight|ectomorph/i.test(category + ' ' + somatotype)
        ? 'Focus on calorie-dense healthy foods, regular protein meals, rice/oats/potatoes, eggs/fish/chicken/tempeh, milk/yogurt, nuts, and olive oil.'
        : 'Use balanced plates: lean protein, whole carbohydrates, vegetables, fruit, healthy fats, and limit sugary drinks.';

    var displayUnit = activeUnit === 'us' ? 'lb' : 'kg';
    var targetText = Number.isFinite(targetWeight) && targetWeight > 0 ? formatNumber(targetWeight, 1) + ' ' + displayUnit : 'Not provided';
    var goalUnitText = Number.isFinite(timeGoalAmount) && timeGoalAmount > 0 ? Math.round(timeGoalAmount) + ' ' + (timeGoal === 'daily' ? 'day' : timeGoal === 'monthly' ? 'month' : 'week') + (Math.round(timeGoalAmount) === 1 ? '' : 's') : (timeGoal === 'daily' ? 'Daily' : timeGoal === 'monthly' ? 'Monthly' : 'Weekly');
    var goalTimeline = 'Enter target weight and time goal to estimate goal timeline';
    var goalHealthy = 'Enter target weight and time goal to check if it is healthy';
    var goalBest = 'Enter target weight to see easy, ideal, and hardest safe options';
    if (Number.isFinite(targetWeight) && targetWeight > 0) {
      var targetKg = activeUnit === 'us' ? targetWeight * 0.45359237 : targetWeight;
      var diffKg = targetKg - weightKg;
      var diffAbs = Math.abs(diffKg);
      var direction = diffKg < 0 ? 'lose' : 'gain';
      goalBest = 'Easy: ' + displayWeightFromKg(0.25, activeUnit) + '/week, Ideal: ' + displayWeightFromKg(0.5, activeUnit) + '/week, Hardest safe: ' + displayWeightFromKg(1, activeUnit) + '/week';
      if (diffAbs < 0.05) {
        goalTimeline = 'Already at target weight';
        goalHealthy = 'Already at target weight';
      } else {
        var weeks = Number.isFinite(timeGoalAmount) && timeGoalAmount > 0 ? (timeGoal === 'daily' ? timeGoalAmount / 7 : timeGoal === 'monthly' ? timeGoalAmount * 4.345 : timeGoalAmount) : Math.ceil(diffAbs / 0.5);
        var perWeek = diffAbs / Math.max(weeks, 1);
        goalTimeline = 'To ' + direction + ' ' + displayWeightFromKg(diffAbs, activeUnit) + ' in ' + goalUnitText + ', aim for about ' + displayWeightFromKg(perWeek, activeUnit) + ' per week.';
        goalHealthy = perWeek <= 1 ? displayWeightFromKg(perWeek, activeUnit) + ' per week is within a safer pace' : displayWeightFromKg(perWeek, activeUnit) + ' per week is not healthy';
      }
    }

    return [
      { label: 'BMI', value: bmi.toFixed(2) },
      { label: 'BMI category', value: category },
      { label: 'Healthy weight range', value: healthyRange },
      { label: 'Difference to healthy range', value: difference },
      { label: 'Health risk', value: healthRisk },
      { label: 'Calories/day to maintain', value: caloriesMaintain },
      { label: 'Calories/day to add weight', value: caloriesGain },
      { label: 'Calories/day to lose weight', value: caloriesLoss },
      { label: 'Waist-to-height ratio', value: Number.isFinite(ratio) ? ratio.toFixed(2) : 'Not provided' },
      { label: 'Waist-to-height status', value: waistStatus },
      { label: 'Body fat estimate', value: bodyFatText },
      { label: 'Body type comment', value: bodyType },
      { label: 'Frame size', value: frameSize },
      { label: 'Fat distribution', value: fatDistribution },
      { label: 'Body shape', value: bodyShape },
      { label: 'Somatotype tendency', value: somatotype },
      { label: 'Neck circumference', value: measurementDisplay(neck, activeUnit) },
      { label: 'Wrist size', value: measurementDisplay(wrist, activeUnit) },
      { label: 'Shoulder width', value: measurementDisplay(shoulder, activeUnit) },
      { label: 'Hip circumference', value: measurementDisplay(hip, activeUnit) },
      { label: 'Goal timeline', value: goalTimeline },
      { label: 'Healthy?', value: goalHealthy },
      { label: 'Best', value: goalBest },
      { label: 'Suggested exercise', value: suggestedExercise },
      { label: 'Suggested foods', value: suggestedFoods },
      { label: 'Unit', value: activeUnit === 'us' ? 'US' : 'SI' },
      { label: 'Name', value: name || 'Not provided' },
      { label: 'Age range', value: ageRangeLabel(age) },
      { label: 'Gender', value: titleCase(gender) },
      { label: 'Activity level', value: ({ sedentary: 'Sedentary', light: 'Light activity', moderate: 'Moderate activity', active: 'Active', veryActive: 'Very active' })[activity] || 'Moderate activity' },
      { label: 'Target weight', value: targetText },
      { label: 'Time goal', value: goalUnitText }
    ];
  }

  function renderBmiDirect() {
    if (!isBmiPage()) return;
    ...
    if (!panel) return;
    var signature = JSON.stringify(rows);
    if (panel.dataset.bmiDirectSignature === signature && panel.classList.contains('bmi-direct-output')) return;
    panel.dataset.bmiDirectSignature = signature;
    panel.hidden = false;

    var bmi = findRow(rows, 'BMI');
    var category = findRow(rows, 'BMI category');
    var healthy = findRow(rows, 'Healthy weight range');
    var difference = findRow(rows, 'Difference to healthy range');
    var risk = findRow(rows, 'Health risk');
    var calories = rows.filter(function (row) { return /^Calories\/day/i.test(row.label); });
    var body = rows.filter(function (row) { return /^(Waist-to-height|Body fat|Body type|Frame size|Fat distribution|Body shape|Somatotype)/i.test(row.label); });
    var measurements = rows.filter(function (row) { return /^(Neck|Wrist|Shoulder|Hip)/i.test(row.label); });
    var goal = rows.filter(function (row) { return /^(Goal|Healthy\?|Best|Target weight|Time goal)/i.test(row.label); });
    var advice = rows.filter(function (row) { return /^(Suggested)/i.test(row.label); });
    var profile = rows.filter(function (row) { return /^(Unit|Name|Age range|Gender|Activity level)/i.test(row.label); });
    ...
    panel.innerHTML =
      '<article class="bmi-rebuilt-result-card bmi-direct-result-card">' +
        ...
        '<div class="bmi-rebuilt-group-grid">' +
          ...
          makeGroup('Body composition', body) +
          makeGroup('Body measurements', measurements) +
          makeGroup('Goal', goal) +
          makeGroup('Advice', advice) +
          makeGroup('Profile', profile) +
        '</div>' +
        '<div class="bmi-rebuilt-actions"><button type="button" class="bmi-rebuilt-btn bmi-copy-btn">Copy</button><button type="button" class="bmi-rebuilt-btn bmi-save-btn">Save</button><button type="button" class="bmi-rebuilt-btn bmi-share-btn">Share</button><button type="button" class="bmi-rebuilt-btn bmi-report-btn">Report</button></div>' +
      '</article>';
    ...
    var shareBtn = panel.querySelector('.bmi-share-btn');
    var reportBtn = panel.querySelector('.bmi-report-btn');
    if (copyBtn) copyBtn.onclick = function () { copyTextFallback(text, copyBtn); };
    if (saveBtn) saveBtn.onclick = function () { downloadText('bmi-result-' + fileStamp() + '.txt', text); tempText(saveBtn, 'Saved'); };
    if (shareBtn) shareBtn.onclick = function () {
      if (navigator.share) navigator.share({ title: 'BMI Calculator Result', text: text }).catch(function () { copyTextFallback(text, shareBtn); });
      else copyTextFallback(text, shareBtn);
    };
    if (reportBtn) reportBtn.onclick = function () { openBmiReport(rows); };

    setTimeout(function () {
      var calc = document.querySelector('main.bmi-calculator-container > .calculator.bmi-calculator-box');
      if (!calc) return;
      var width = Math.round(calc.getBoundingClientRect().width);
      if (width > 0 && window.innerWidth > 850) {
        panel.style.setProperty('width', width + 'px', 'important');
  ...
  function scheduleRender() {
    renderBmiDirect();
    setTimeout(renderBmiDirect, 80);
    setTimeout(renderBmiDirect, 240);
    setTimeout(renderBmiDirect, 650);
  }

  function install() {
    if (!isBmiPage()) return;
    window.calculateBMI = function () { scheduleRender(); };
    window.calculateBmi = window.calculateBMI;
    document.addEventListener('input', function (event) {
      if (event.target && event.target.matches('#weight,#height,#bmiGender,#bmiActivityLevel,#bmiName,#bmiAge,#waist,#bmiNeck,#bmiWrist,#bmiShoulder,#bmiHip,#bmiTargetWeight,#bmiTimeGoalAmount,#bmiTimeGoal')) scheduleRender();
    }, true);
    document.addEventListener('change', function (event) {
      if (event.target && event.target.matches('#weight,#height,#bmiGender,#bmiActivityLevel,#bmiName,#bmiAge,#waist,#bmiNeck,#bmiWrist,#bmiShoulder,#bmiHip,#bmiTargetWeight,#bmiTimeGoalAmount,#bmiTimeGoal,#unitToggleBtn')) scheduleRender();
    }, true);
    document.addEventListener('click', function (event) {
      if (event.target && event.target.id === 'unitToggleBtn') setTimeout(scheduleRender, 100);
    }, true);
    [50, 250, 800, 1500].forEach(function (delay) { setTimeout(scheduleRender, delay); });
  }
