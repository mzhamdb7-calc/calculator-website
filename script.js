...
/* =====================================================
   COMPOUND INTEREST: split reference items into own boxes
   Removes the grouped References box only on compound page
===================================================== */
(function () {
  "use strict";
  ...
  function isCompoundPage() {
    const h1 = document.querySelector("h1");
    return h1 && h1.textContent.toLowerCase().includes("compound");
  }

  function createCompoundRefBox(title, text) {
    const box = document.createElement("div");
    box.className = "compound-reference-item";
  ...
  function splitCompoundReferences() {
    if (!isCompoundPage()) return;

    const instructionBox = document.querySelector("main .instruction-box");
    if (!instructionBox) return;

    /* Remove old grouped reference box */
    instructionBox.querySelectorAll(".reference-box").forEach(function (box) {
      box.remove();
    });

    /* Prevent duplicate boxes */
    if (instructionBox.querySelector(".compound-reference-item")) return;

    const references = [
      ["Formula reference", "A = P(1 + r/n)ⁿᵗ"],
      ["Interest reference", "Compound Interest = A − P"],
      ["Meaning", "P = principal, r = annual rate, n = compounding frequency, t = years."],
      ["Example reference", "P = 1000, r = 5%, t = 10 years, n = 12 gives about 1,647.01 future value."]
    ];

    references.forEach(function (item) {
      instructionBox.appendChild(createCompoundRefBox(item[0], item[1]));
    });
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function () {
      splitCompoundReferences();
      setTimeout(splitCompoundReferences, 300);
    });
  } else {
    splitCompoundReferences();
    setTimeout(splitCompoundReferences, 300);
