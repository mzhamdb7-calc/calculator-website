...
/* ChatGPT update: live graph for scientific calculator, replacing result box */
(function () {
  function isScientificPage() {
    return document.body && (document.body.dataset.page === "scientific" || location.pathname.includes("scientific-calculator"));
  }
  ...
  function buildFunction(expression) {
    const safe = normaliseExpression(expression);
    const allowed = /^[0-9+\-*/%().,\s*xMathPIEhlgocinsqrtab]+$/i;
    if (!allowed.test(safe)) throw new Error("Unsupported expression");
    return Function("x", '"use strict"; return (' + safe + ');');
  }
  ...
  function drawEmpty(ctx, canvas, message) {
    ...
    ctx.fillText(message || "Enter an expression to draw the graph", canvas.width / 2, canvas.height / 2);
  }

  function drawGraph(expression) {
    const canvas = document.getElementById("scientificGraphCanvas");
    const status = document.getElementById("scientificGraphStatus");
    if (!canvas) return;
    ...
    if (!input) {
      drawEmpty(ctx, canvas, "Enter an expression to draw the graph");
    ...
    let fn;
    try {
      fn = buildFunction(input);
    } catch (error) {
      drawEmpty(ctx, canvas, "Unsupported expression");
      if (status) status.textContent = "Use numbers, x, +, -, *, /, ^, brackets, sqrt(), sin(), cos(), tan(), log(), ln(), abs(), pi, and e.";
      return;
    }
    ...
    for (let i = 0; i <= samples; i++) {
      ...
      let y;
      try { y = Number(fn(x)); } catch (error) { y = NaN; }
      if (Number.isFinite(y) && Math.abs(y) < 1e6) points.push({ x: x, y: y });
      else points.push({ x: x, y: NaN });
    }

    const finiteY = points.map(p => p.y).filter(Number.isFinite);
    if (!finiteY.length) {
      drawEmpty(ctx, canvas, "No graphable values in this range");
      if (status) status.textContent = "No graphable values found from x = -10 to x = 10.";
      return;
    }

    let yMin = Math.min.apply(null, finiteY);
    let yMax = Math.max.apply(null, finiteY);
    if (yMin === yMax) { yMin -= 5; yMax += 5; }
    const yPad = (yMax - yMin) * 0.12;
    yMin -= yPad;
    yMax += yPad;

    function sx(x) { return pad + (x - xMin) / (xMax - xMin) * (width - pad * 2); }
    function sy(y) { return height - pad - (y - yMin) / (yMax - yMin) * (height - pad * 2); }
    ...
    ctx.lineWidth = 1.4;
    ctx.beginPath();
    if (xMin <= 0 && xMax >= 0) { ctx.moveTo(sx(0), pad); ctx.lineTo(sx(0), height - pad); }
    if (yMin <= 0 && yMax >= 0) { ctx.moveTo(pad, sy(0)); ctx.lineTo(width - pad, sy(0)); }
    ctx.stroke();
    ...
    points.forEach(function (p) {
      if (!Number.isFinite(p.y)) { drawing = false; return; }
      const px = sx(p.x);
      const py = sy(p.y);
    ...
    ctx.textAlign = "center";
    ctx.fillText("-10", sx(-10), height - 10);
    ctx.fillText("0", sx(0), height - 10);
    ctx.fillText("10", sx(10), height - 10);
    ctx.textAlign = "left";
    ctx.fillText("y max " + yMax.toFixed(2), pad, 18);
    ctx.fillText("y min " + yMin.toFixed(2), pad, height - 12);

    if (status) status.textContent = "Live graph for y = " + input + " shown from x = -10 to x = 10.";
  }

  function installScientificLiveGraph() {
    if (!isScientificPage()) return;
    const resultBox = document.getElementById("extraCalcResult");
    if (resultBox) resultBox.remove();

    const input = document.getElementById("scientificExpression");
    if (!input) return;

    let graphTimer = null;
    function scheduleGraph() {
      clearTimeout(graphTimer);
      graphTimer = setTimeout(function () { drawGraph(input.value); }, 120);
    }

    input.addEventListener("input", scheduleGraph);
    input.addEventListener("change", scheduleGraph);
    setTimeout(function () { drawGraph(input.value); }, 100);
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", installScientificLiveGraph);
  } else {
    installScientificLiveGraph();
  }
  setTimeout(installScientificLiveGraph, 500);
  setTimeout(installScientificLiveGraph, 1200);
})();

/* ChatGPT update: scientific input buttons for live graph */
(function () {
  function getScientificInput() {
    return document.getElementById("scientificExpression");
  }
  function triggerScientificGraph(input) {
    if (!input) return;
    input.dispatchEvent(new Event("input", { bubbles: true }));
  }
  window.focusScientificInput = function () {
    const input = getScientificInput();
    if (!input) return;
    input.focus();
    const len = input.value.length;
    try { input.setSelectionRange(len, len); } catch (e) {}
    triggerScientificGraph(input);
  };
  window.appendScientific = function (value) {
    ...
    const next = start + String(value).length;
