...
/* =====================================================
   BMI LAYOUT FIX:
   Put Weight / Height / Waist in one box,
   beside Age / Sex / BMI cut-off box
===================================================== */
(function () {
  "use strict";
  ...
  function movePair(box, labelId, inputId) {
    const label = el(labelId);
    const input = el(inputId);

    if (label && !box.contains(label)) {
      box.appendChild(label);
    }

    if (input && !box.contains(input)) {
      box.appendChild(input);
    }
  }

  function setupBmiTwoBoxLayout() {
    if (!isBmiPage()) return;

    const calculator = document.querySelector(".bmi-calculator-box") || document.querySelector(".calculator");
    const titleRow = document.querySelector(".bmi-title-row");
    const result = el("bmiResult");

    if (!calculator || !titleRow) return;

    let row = document.querySelector(".bmi-input-two-column");

    if (!row) {
      row = document.createElement("div");
      row.className = "bmi-input-two-column";
      titleRow.insertAdjacentElement("afterend", row);
    }

    let bodyBox = document.querySelector(".bmi-body-input-box");

    if (!bodyBox) {
      bodyBox = document.createElement("div");
      bodyBox.className = "bmi-body-input-box";
      bodyBox.innerHTML = '<h2 class="bmi-box-title">Body details</h2>';
    }

    let profileBox = document.querySelector(".bmi-extra-profile-box");

    if (!profileBox) {
      profileBox = document.createElement("div");
      profileBox.className = "bmi-extra-profile-box";
    }

    if (!profileBox.querySelector(".bmi-box-title")) {
      profileBox.insertAdjacentHTML("afterbegin", '<h2 class="bmi-box-title">Profile</h2>');
    }

    movePair(bodyBox, "weightLabel", "weight");
    movePair(bodyBox, "heightLabel", "height");
    movePair(bodyBox, "waistLabel", "waist");

    if (!row.contains(bodyBox)) {
      row.appendChild(bodyBox);
    }

    if (!row.contains(profileBox)) {
    ...
    if (result && result.previousElementSibling !== row) {
      row.insertAdjacentElement("afterend", result);
    }
  }

  function runBmiLayoutFix() {
    setupBmiTwoBoxLayout();

    setTimeout(setupBmiTwoBoxLayout, 50);
    setTimeout(setupBmiTwoBoxLayout, 250);
    setTimeout(setupBmiTwoBoxLayout, 700);
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", runBmiLayoutFix);
  } else {
    runBmiLayoutFix();
  }

  window.addEventListener("load", runBmiLayoutFix);

  document.addEventListener("change", function (event) {
    if (
      event.target &&
      ["bmiAgeGroup", "bmiSex", "bmiEthnicity", "weight", "height", "waist"].includes(event.target.id)
    ) {
      setTimeout(setupBmiTwoBoxLayout, 0);
    }
  });
})();